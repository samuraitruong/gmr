<%@ Page Title="" Language="C#" MasterPageFile="~/Areas/Administration/Views/Shared/CMS.Master" Inherits="System.Web.Mvc.ViewPage<GMR.App.Areas.Administration.Models.EditGroupPermissionModel>" %>
                 ...
                 var permissionItem = item.Permissions.Where(p => p.GroupID == Model.GroupId).FirstOrDefault();
                     %>
										<tr class="<%: index++%2==0?"first":"second" %>">
											<%--<td><%: index %></td>--%>
											<td><%: item.Name%></td>
											<td><%=Html.CheckBox("Full_" + item.ModuleID,permissionItem!= null && permissionItem.Full.HasValue&& permissionItem.Full.Value?true:false, new { @class="CheckedItem"})%></td>
                                            <td><%=Html.CheckBox("Create_" + item.ModuleID, permissionItem != null && permissionItem.Create.HasValue && permissionItem.Create.Value ? true : false, new { @class = "CheckedItem" })%></td>
                                            <td><%=Html.CheckBox("Read_" + item.ModuleID, permissionItem != null && permissionItem.Read.HasValue && permissionItem.Read.Value ? true : false, new { @class = "CheckedItem" })%></td>
                                            <td><%=Html.CheckBox("Edit_" + item.ModuleID, permissionItem != null && permissionItem.Edit.HasValue && permissionItem.Edit.Value ? true : false, new { @class = "CheckedItem" })%></td>
											<td style="width: 96px;">
												<div class="actions">
								...
								<div class="table_menu">
									<ul class="left">
										<li><a href="<%=Url.Action("Index","Permission") %>" class="button add_new"><span><span>Quay Lại</span></span></a></li>
									</ul>
									<ul class="right">
