        ...
        $(".CheckedItem").click(function () {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $(".CheckedItemX").attr("disabled", "disabled").attr("readonly","true");
                }
            });
...
<!--[if !IE]>start section<![endif]-->	
	<div class="section table_section">
		<!--[if !IE]>start title wrapper<![endif]-->
		<div class="title_wrapper">
			<h2>Phân quyền cho cá nhân : <%=Model.User.FullName %> (Là quản trị hệ thống : <%=Html.CheckBox("SA", Model.User.IsSA()) %>)</h2>
			<span class="title_wrapper_left"></span>
			<span class="title_wrapper_right"></span>
		</div>
		<!--[if !IE]>end title wrapper<![endif]-->
                 ...
                 var permissionItem = Model.User.Permissions.Where(p => p.ModuleID == item.ModuleID).FirstOrDefault();
                     %>
										<tr class="<%: index++%2==0?"first":"second" %>">
											<%--<td><%: index %></td>--%>
											<td><%: item.Name%></td>
											<td><%=Html.CheckBox("Full_" + item.ModuleID,permissionItem!= null && permissionItem.Full.HasValue&& permissionItem.Full.Value?true:false, new { @class="CheckedItemX"})%></td>
                                            <td><%=Html.CheckBox("Create_" + item.ModuleID, permissionItem != null && permissionItem.Create.HasValue && permissionItem.Create.Value ? true : false, new { @class = "CheckedItemX" })%></td>
                                            <td><%=Html.CheckBox("Read_" + item.ModuleID, permissionItem != null && permissionItem.Read.HasValue && permissionItem.Read.Value ? true : false, new { @class = "CheckedItemX" })%></td>
                                            <td><%=Html.CheckBox("Edit_" + item.ModuleID, permissionItem != null && permissionItem.Edit.HasValue && permissionItem.Edit.Value ? true : false, new { @class = "CheckedItemX" })%></td>
											<td style="width: 96px;">
												<div class="actions">
